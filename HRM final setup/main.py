...
@login_required
def hr_again():
    user = Employee.query.filter_by().first()
    login_user(user)
...
def hr_leave_requests():
    ...
    return render_template('hr_leave_requests.html', leave_requests=leave_requests)
...
def department_leave_requests():
    ...
    leave_requests = LeaveRequest.query.all()
    return render_template('department_leave_requests.html', leave_requests=leave_requests)
